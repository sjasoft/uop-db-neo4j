...
class Neo4jCollection(db_coll.DBCollection):
    ...
    def insert(self, **object_data):
        ...
        with self._db.get_session() as session:
            result = session.execute_write(self._create_node, label, object_data, tenant_id)
            return result
    ...
    def find(self, criteria=None, only_cols=None, order_by=None, limit=None, ids_only=False):
        ...
        tenant_id = self._db._tenant_id
        with self._db.get_session() as session:
            result = session.execute_read(self._match_nodes, label, criteria, only_cols, order_by, limit, tenant_id)
            return result
    ...
    def update(self, criteria, mods, partial=True):
        ...
        tenant_id = self._db._tenant_id
        with self._db.get_session() as session:
            session.execute_write(self._update_nodes, label, criteria, mods, partial, tenant_id)
